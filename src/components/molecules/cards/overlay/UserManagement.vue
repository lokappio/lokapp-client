        ...
        <v-container class="pa-0">
            ...
            <v-list subheader class="mx-0 users-list" two-line>
                ...
                <v-list-item class="px-0" v-for="user in users" :key="user.userId">
                    ...
                    <v-list-item-action>
                        <v-select hide-details="true" :disabled="isUserUpdateRoleDisabled(user) || user.userId === myself.userId" light solo v-model="user.role" :items="roles" item-text="text" item-value="value" @change="updateRole(user)">
