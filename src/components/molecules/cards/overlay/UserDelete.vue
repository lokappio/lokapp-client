      ...
      <v-container class="pa-0">
        <v-row class="mt-0 pb-0">
          <v-col cols="12" class="pb-0 px-0">
            <action-button block :loading="loading" :handler="deleteUser" :text="$t('user_delete.confirm_button').toString()"/>
          </v-col>
        </v-row>

        <v-row class="mt-2 pb-0">
          <v-col cols="12" class="pb-0 px-0 text-center">
            <v-btn text @click="closeOverlay" color="primary">{{ $t('user_delete.cancel_button').toString() }}</v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-card-actions>
